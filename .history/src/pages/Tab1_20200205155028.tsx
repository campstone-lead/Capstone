    ...
    if(this.props.user['status']==='booker'){
      ...
      this.props.venues.forEach(async(venue,index)  => {
        ...
        if(index===0){
          ...
          rec=this.state.recommendationsBooker.slice();
          rec=rec.filter((el)=>(el['id']!==artist.id&&(el['id'].recommendations['score']<artist.recommendations.score)))
         // rec.push(artist)
          console.log('filtered here=>>>',rec)
          this.setState({recommendationsBooker:rec})
        }
        })
      })
