...
class VenueSinglePage extends React.Component<
    ...
    render() {
        ...
        if (this.props.events !== undefined)
            filteredEvents = this.props.events.filter(
                event => event['venueId'] === this.props.venue['id']
            );
        return (
            <IonPage>
                <IonHeader mode="ios">
                    ...
                    >
                        ...
                        <VenueSingleComponent
                            ...
                            isOwner={
                                ...
                                this.props.user['id'] === this.props.venue['bookerId']
                            }
                        />
                        ...
                        {this.props.user['status'] !== 'booker' &&
                            filteredEvents.length !== 0 ? (
                                ...
                                filteredEvents.length === 0 && (
                                    <h3>There are no current events happening here.</h3>
                                )
                            )}

                        {this.props.user['status'] !== 'booker' &&
                            filteredEvents.length !== 0 &&
                            (this.state.localStatus && this.state.sender === 'booker' ? (
                                this.state.localStatus === 'pending' ? (
                                    <h4 color="black">
                                        You have an incoming request from a booker
                  </h4>
                                ) : this.state.localStatus.length === 0 ? null : (
                                    <h4 color="black">
                                        This is{' '}
                                        {this.state.localStatus === 'booked'
                                            ? 'an aproved'
                                            : 'a declined'}{' '}
                                        request by the {this.props.booker['booker']['firstName']}{' '}
                                        {this.props.booker['booker']['lastName']} for the selected
                                        event.
                  </h4>
                                )
                            ) : (
                                    this.state.sender === 'artist' &&
                                    this.state.localStatus &&
                                    (this.state.localStatus === 'pending' ? (
                                        <h4 color="black">
                                            You request for {this.props.booker['booker']['firstName']}{' '}
                                            {this.props.booker['booker']['lastName']} is pending...
                  </h4>
                                    ) : this.state.localStatus === 'booked' ? (
                                        <h4 color="black">
                                            You request for {this.props.booker['booker']['firstName']}{' '}
                                            {this.props.booker['booker']['lastName']} has been approved.
                  </h4>
                                    ) : (
                                                <h4 color="black">
                                                    You request for {this.props.booker['booker']['firstName']}{' '}
                                                    {this.props.booker['booker']['lastName']} has been declined.
                  </h4>
                                            ))
                                ))}
                        ...
                        {this.props.user['status'] !== 'booker' &&
                            ...
                            (this.state.sender && this.state.sender === 'booker' ? (
                                <div>
                                    ...
                                    filteredEvents.length !== 0 && (
                                        <IonButton
                                            style={{ fontSize: '15.5px', margin: '20px' }}
